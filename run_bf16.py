...
def main():
    ...
    is_llama = any(isinstance(m, LlamaDecoderLayer) for m in model.modules())

    if is_llama and not configs.only_eval:
        # FSDP is only beneficial for large models (e.g. Llama) that don't fit in single GPU memory
        ...
        # GPT-2 fits in single GPU memory; DDP is faster than FSDP for small models
        ...
        if not configs.only_eval:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if step == 0 and wandb_run and rank == 0:
                    ...
                    text_str = ""
                    for data_idx in range(min(cur_bs, 2)):  # limit to 2 samples to avoid wandb socket overflow
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=configs.bf16):
